...
public class Assignment5 {
	...
	private boolean KMPsearch = false;
	private boolean BMsearch = false;
	public Assignment5() {
		initialise();
	}
	...
	private void onAlgorithmRun() {
		try {
			...
			if (algorithm.equals("Huffman coding")) {
				...
				System.out.println("Time taken for LZ to run: "+ elapsedTime);
				// write out the encoded text.
				...
				ng.Ngramsd(text);
				List<Float> ex = ng.findCharProbs("Hurihia to aroaro ki te ra tukuna to atarangi kia taka ki muri i a koe");
				...
				float logprob = ng.calcTotalLogProb(ex);
				List<Float> eng = ng.findCharProbs("Turn your face to the sun and the shadows fall behind you, translation");
				float Englogprob = ng.calcTotalLogProb(eng);
				System.out.println("final log base 2 probability for Te Reo version: "+logprob);
				System.out.println("final log base 2 probability for English version: "+Englogprob);
			}
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	private void initialise() {
		...
		JButton BM = new JButton("BMSearch");
		...
		searchField.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int index = 0;
				if(KMPsearch & !BMsearch) {
					System.out.println("Now using KMP");
					long startTime = System.nanoTime();
					index = new KMP(pattern, text).search(pattern, text);
					long elapsedTime = System.nanoTime() - startTime;
					System.out.println("Time taken for KMP to run: "+elapsedTime);
				}
				else if(!KMPsearch & !BMsearch) {
					System.out.println("Now using Brute");
					long startTime = System.nanoTime();
					index = new BruteForce(pattern, text).search(pattern, text);
					long elapsedTime = System.nanoTime() - startTime;
					System.out.println("Time taken for BruteForce to run: "+elapsedTime);
				}else if(!KMPsearch & BMsearch){
					char[] pat = pattern.toCharArray();
					char[] txt = text.toCharArray();
					System.out.println("Now using BoyerMoore search");
					long startTime = System.nanoTime();
					BoyerMooreSearch bm = new BoyerMooreSearch();
					bm.boyerMoore(pattern,3, text,256);
					long elapsedTime = System.nanoTime() - startTime;
					System.out.println("Time taken for Boyer Moore to run: "+elapsedTime);
				}
